...
void listener(int event_type, dxf_const_string_t symbol_name,
	...
	if (event_type == DXF_ET_TRADE) {
		...
		for (; i < data_count; ++i) {
			...
			wprintf(L", exchangeCode=%c, price=%f, size=%i, tick=%i, change=%f, day volume=%.0f, c=%d}\n",
	...
	if (event_type == DXF_ET_TRADE_ETH) {
		...
		for (; i < data_count; ++i) {
			print_timestamp(trades[i].time);
			wprintf(L", exchangeCode=%c, flags=%d, price=%f, size=%i, day volume=%.0f, c=%d}\n",
				trades[i].exchange_code, trades[i].raw_flags, trades[i].price, trades[i].size, trades[i].day_volume, trades[i].is_composite);
		}
	}
