...
void listener(int event_type, dxf_const_string_t symbol_name, const dxf_event_data_t* data, int data_count,
	...
	if (event_type == DXF_ET_TRADE) {
		...
		for (; i < data_count; ++i) {
			wprintf(L"time=%lld, exchange code=%C, price=%.10f, size=%.10f, tick=%ld, change=%.10f, day id=%d, day volume=%.10f, scope=%d\n",
	...
	while (g_iteration_count--) {
		...
		for (int i = 0; i < g_symbols_size; ++i) {
			...
			if (data) {
				...
				wprintf(
					L"\nSymbol: %ls; time=%lld, exchange code=%c, price=%.10f, size=%.10f, tick=%ld, change=%.10f, day id=%d, day "
					L"volume=%.10f, scope=%d\n",
