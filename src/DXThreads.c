...
void dx_mark_thread_master(void) { g_master_thread_id = dx_get_thread_id(); }

int dx_is_thread_master(void) { return dx_compare_threads(dx_get_thread_id(), g_master_thread_id); }
...
void* dx_get_thread_data(dx_key_t key) { return pthread_getspecific(key); }

dx_thread_t dx_get_thread_id() { return pthread_self(); }

int dx_compare_threads(dx_thread_t t1, dx_thread_t t2) { return (pthread_equal(t1, t2) != 0); }
...
int dx_set_thread_data_no_ehm(dx_key_t key, const void* data) { return (pthread_setspecific(key, data) == 0); }
...
static void dx_call_keys_destructor(void *arg) {
	int i;
	if (g_key_destructors.count == 0) return;
	if (!dx_mutex_lock(&g_key_destructors.mutex)) return;
...
static void dx_add_key_destructor(dx_key_t key, void (*destructor)(void *)) {
	if (g_key_destructors.size == 0) return;
	if (!dx_mutex_lock(&g_key_destructors.mutex)) return;
...
static void dx_remove_key_destructor(dx_key_t key) {
	...
	for (i = 0; i < g_key_destructors.count; i++) {
		if (g_key_destructors.destructors[i].key == key) {
			dx_memmove(&g_key_destructors.destructors[i], &g_key_destructors.destructors[i + 1],
					   sizeof(*g_key_destructors.destructors) * (g_key_destructors.count - i - 1));
			g_key_destructors.count--;
			break;
		}
	}
...
void *dx_get_thread_data(dx_key_t key) { return TlsGetValue(key); }

dx_thread_t dx_get_thread_id() { return GetCurrentThread(); }

int dx_compare_threads(dx_thread_t t1, dx_thread_t t2) { return t1 == t2; }
...
int dx_set_thread_data_no_ehm(dx_key_t key, const void *data) { return dx_set_thread_data(key, data); }
